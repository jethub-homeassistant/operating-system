...
function prepare_rauc_signing() {
    ...
    if [ ! -f "${key}" ]; then
        ...
        "${BR2_EXTERNAL_JHOS_PATH}"/scripts/generate-signing-key.sh "${cert}" "${key}"
    fi
}
...
function write_rauc_config() {
    ...
    (
        "${HOST_DIR}/bin/tempio" \
            -template "${BR2_EXTERNAL_JHOS_PATH}/ota/system.conf.gtpl"
...
function install_rauc_certs() {
    ...
    if [ "${DEPLOYMENT}" == "development" ]; then
        ...
        cp "${BR2_EXTERNAL_JHOS_PATH}/ota/dev-ca.pem" "${TARGET_DIR}/etc/rauc/keyring.pem"
    else
        cp "${BR2_EXTERNAL_JHOS_PATH}/ota/rel-ca.pem" "${TARGET_DIR}/etc/rauc/keyring.pem"
    fi
    ...
    if ! openssl verify -CAfile "${BR2_EXTERNAL_JHOS_PATH}/ota/dev-ca.pem" -no-CApath "${cert}"; then
